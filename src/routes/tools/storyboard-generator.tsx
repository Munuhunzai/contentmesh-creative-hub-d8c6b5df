...
export function StoryboardGeneratorPage() {
  ...
  const handleAssistantSubmit = (query: string) => {
    ...
    // Dynamically apply assistant modification to output scenes
    setTimeout(() => {
  ...
  return (
    <SiteLayout>
      ...
      {step === "script" && (
        <section className="mx-auto max-w-5xl px-4 sm:px-6 py-12 sm:py-20 w-full max-w-full space-y-12">
          ...
          <div className="text-center space-y-3">
            <h1 className="font-display text-2xl sm:text-4xl lg:text-5xl font-black tracking-tight text-foreground">
          ...
          <div className="space-y-4 pt-4">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-xs sm:text-sm font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {EXAMPLE_SCRIPTS.map((ex) => (
                ...
                >
                  ...
                  <h4 className="font-display text-sm font-bold text-foreground group-hover:text-[#FF5A1F] transition-colors">
                    {ex.title}
                  </h4>
      ...
      {/* ─────────────────────────────────────────────────────────────────── */}
      {/* ── STEP 3: STUDIO & AI ASSISTANT (MATCHES SCREENSHOT 2) ─────────── */}
      {/* ─────────────────────────────────────────────────────────────────── */}
      {step === "studio" && (
        <section className="mx-auto max-w-7xl px-3 sm:px-6 pb-20 w-full max-w-full overflow-hidden" id="generator-workspace">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full max-w-full items-start">
            ...
            {/* ── LEFT SIDEBAR: AI ASSISTANT BOX FIXED ON LEFT (MATCHES SCREENSHOT 2) ── */}
            <div className="lg:col-span-4 xl:col-span-4 flex flex-col justify-between space-y-4 w-full max-w-full lg:sticky lg:top-20">
              ...
              {/* ── FIXED AI ASSISTANT CHAT/MODIFIER BOX (MATCHES SCREENSHOT 2) ── */}
              <div className="rounded-2xl sm:rounded-3xl border border-border/80 bg-card p-3.5 sm:p-4 shadow-glass backdrop-blur-xl max-w-full space-y-3">
                <div className="flex items-center justify-between border-b border-border/40 pb-2">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-foreground flex items-center gap-1.5">
                    <Sparkles className="h-3.5 w-3.5 text-[#FF5A1F]" /> AI Scene Assistant
                  </span>
                    ...
                    {assistantLogs.map((log, idx) => (
                      <div
                        ...
                        className={`p-2 rounded-xl text-[11px] ${
                          log.sender === "user"
                            ? "bg-[#FF5A1F]/10 text-foreground border border-[#FF5A1F]/20 ml-4 text-right"
                ...
                <div className="flex flex-wrap gap-1">
                  {[
                    ...
                    <button
                      ...
                      className="rounded-full bg-secondary/50 px-2 py-0.5 text-[9px] font-semibold text-muted-foreground hover:bg-[#FF5A1F]/10 hover:text-[#FF5A1F] transition-all"
                    >
                      + {chip}
                    </button>
                  ))}
                ...
                <div className="space-y-2">
                  <textarea
                    ...
                    placeholder="Ask assistant to modify scenes (e.g. 'Add dramatic push-in to all scenes')..."
                    className="w-full rounded-xl border border-input bg-background/80 px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-[#FF5A1F]/40 leading-relaxed font-mono break-words whitespace-pre-wrap"
                  />
            ...
            {/* ── RIGHT STUDIO PANEL (SCENE BOXES CONTAINING ONLY SCENE NUMBER & SCENE) ── */}
            <div className="lg:col-span-8 xl:col-span-8 space-y-4 min-w-0 max-w-full">
              {output && (
                <div className="space-y-4 w-full">
                  ...
                  {/* ── GENERATED SCENE BOXES (CONTAINING ONLY SCENE NUMBER & SCENE PROMPT, NOTHING ELSE) ── */}
