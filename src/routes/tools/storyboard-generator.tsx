    ...
    try {
      const res = await fetch("/api/assistant-modify", {
        ...
        body: JSON.stringify({
          ...
          chatHistory: updatedLogs,
        }),
      });

      if (!res.ok) {
        throw new Error("Assistant service failed.");
      }

      const data = await res.json();
      const actionDesc = data.actionSummary || "Updated storyboard package";
      ...
      if (data.updatedOutput && data.updatedOutput.scenes) {
        ...
        const updatedHistory = storyHistory.map((item) =>
          item.script === form.script || item.output?.project?.title === output.project?.title
            ? {
                ...item,
                script: data.updatedForm?.script || item.script,
                visualStyle: data.updatedForm?.visualStyle || item.visualStyle,
